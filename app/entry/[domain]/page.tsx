...
export default function EntryForm({ params }: { params: { domain?: string } }) {
  ...
  useEffect(() => {
    ...
    checkUser()
  }, [router, params.domain])

  const loadEntry = async () => {
    try {
      const res = await fetch(`/api/entries/${params.domain}`)
      if (res.status === 401) {
        router.push('/')
        return
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const method = params.domain ? 'PUT' : 'POST'
      const url = params.domain 
        ? `/api/entries/${entry.domain}`
        : '/api/entries'
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/entries/${entry.domain}`, { method: 'DELETE' })
    ...
    <div className="min-h-screen bg-[#0a0a0f] text-[#e8e8f0]">
      <div className="container mx-auto px-4 py-8">
        <h1 className="text-2xl font-bold mb-6">
          {params.domain ? 'Edit Entry' : 'New Entry'}
        </h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-4 mt-6">
            ...
            >
              {loading ? 'Saving...' : (params.domain ? 'Update' : 'Create')}
            </button>
            ...
            {params.domain && (
              <button
                ...
                className="px-6 py-2 bg-[#ef4444] rounded-lg hover:bg-[#dc2626]"
              >
                Delete
              </button>
            )}
